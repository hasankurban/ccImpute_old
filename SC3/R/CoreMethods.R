...
sc3.SingleCellExperiment <- function(object, ks, gene_filter, pct_dropout_min, pct_dropout_max, d_region_min, 
    ...
    object <- sc3_kmeans(object, ks, FALSE)
...
#' Can also be a single integer.
#' @param useFastClust - if True uses fast community clustering, otherwise uses k means.
#' 
#' @return an object of \code{SingleCellExperiment} class
#' 
#' @importFrom doRNG %dorng%
...
sc3_kmeans.SingleCellExperiment <- function(object, ks, useFastClust) {
  # @importFrom mbkmeans mini_batch
  # @importFrom scran mini_batch
  # @import DCEM
  # @import clValid
    if (is.null(ks)) {
    ...
    labs <- foreach::foreach(i = 1:nrow(hash.table)) %dorng% {
        try({
            ...
            if(useFastClust){
                ...
                # for (val in 1: 500)
                # {
                #     cluster <- DCEM::dcem_star_train(data = transf[, 1:hash.table$n_dim[i]], num_clusters = hash.table$ks[i],seeding="rand")$membership
                #     val <- clValid::dunn(dist_out, cluster)
                #     if(val > best_val){
                #         best_val <-val
                ...
                # dist_out <- dist(transf[, 1:hash.table$n_dim[i]], method = "euclidean", diag = FALSE, upper = FALSE, p = 2)
                # 
                # hclust_avg<-hclust(dist_out, method = "mcquitty", members = NULL)
                # cutree(hclust_avg, k = hash.table$ks[i])
            }
        })
    }
